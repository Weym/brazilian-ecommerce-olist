...
class TestFeatures(unittest.TestCase):
    def test_import_works(self):
        ...
        self.assertIsNotNone(PRE_DELIVERY_FEATURES)
        self.assertIsNotNone(FORBIDDEN_FEATURES)
        self.assertIsNotNone(TARGET_COLUMN)

    def test_pre_delivery_features_has_13_columns(self):
        """Test 2: PRE_DELIVERY_FEATURES must be a list with exactly 13 strings."""
        from src.features import PRE_DELIVERY_FEATURES
        self.assertIsInstance(PRE_DELIVERY_FEATURES, list, "PRE_DELIVERY_FEATURES deve ser uma lista")
        self.assertEqual(len(PRE_DELIVERY_FEATURES), 13, (
            f"PRE_DELIVERY_FEATURES deve ter 13 colunas, tem {len(PRE_DELIVERY_FEATURES)}"
        ))
        for col in PRE_DELIVERY_FEATURES:
            self.assertIsInstance(col, str, f"Todos os itens devem ser strings, encontrado: {type(col)}")

    def test_required_forbidden_columns_present(self):
    ...
    def test_no_forbidden_in_pre_delivery(self):
        ...
        self.assertFalse(leakage, f"LEAKAGE DETECTADO: {leakage}")

    def test_seller_customer_distance_km_present(self):
        """Test 6: seller_customer_distance_km must be in PRE_DELIVERY_FEATURES."""
        from src.features import PRE_DELIVERY_FEATURES
        self.assertIn("seller_customer_distance_km", PRE_DELIVERY_FEATURES, (
